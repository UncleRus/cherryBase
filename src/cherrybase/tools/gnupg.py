...
2013-06-11 fixed multiple shell injection vulnerabilities
Ruslan V. Uss, unclerus@gmail.com

"""
__version__ = "0.3.1"
__author__ = "Ruslan V. Uss"
__date__ = "$11-Jun-2013 23:10:10$"
...
def _copy_data(instream, outstream):
    ...
    sent = 0
    enc = getattr(sys.stdin, 'encoding', 'ascii')
    while True:
...
class ListKeys(list):
    ...
    def key(self, args):
        vars = ('type', 'trust', ' length', 'algo', 'keyid', 'date', 'expires', 'dummy', 'ownertrust', 'uid')
        self.curkey = {var: args[i] for i, var in enumerate(vars)}

        self.curkey['uids'] = [self.curkey['uid']] if self.curkey['uid'] else []
        del self.curkey['uid']
...
class GPG(object):
    ...
    def _open_subprocess(self, args, passphrase = False):
        ...
        cmd = [self.gpgbinary, '--status-fd', '2', '--no-tty', '--lock-multiple', '--no-permission-warning']
        if self.gnupghome:
            cmd += ['--homedir', self.gnupghome]
        if self.keyring:
            cmd += ['--no-default-keyring', '--keyring', self.keyring]
        if passphrase:
    ...
    def delete_keys(self, fingerprints, secret = False):
        args = ['--batch', '--delete-%s' % ('secret-key' if secret else 'key')]
        args.extend (fingerprints if _is_sequence(fingerprints) else [fingerprints])
    ...
    def export_keys(self, keyids, secret = False):
        ...
        args = ['--armor', '--export%s' % ('-secret-key' if secret else '')]
        args.extend (keyids if _is_sequence(keyids) else [keyids])
        p = self._open_subprocess(args)
    ...
    def list_keys(self, secret = False):
        ...
        args = [
            '--list-%s' % ('secret-keys' if secret else 'keys'),
            '--fixed-list-mode',
            '--fingerprint',
            '--with-colons'
        ]
    ...
    def _decrypt(self, args, file, always_trust, passphrase, output):
        if output:  # write the output to a file with the specified name
            if os.path.exists(output):
                os.remove(output) # to avoid overwrite confirmation message
            args += ['--output', output]
    ...
    def decrypt_file(self, file, always_trust = False, passphrase = None,
                     output = None):
        return self._decrypt (["--decrypt"], file, always_trust, passphrase, output)
    ...
    def decrypt_verify_file(self, file, encoder_key, always_trust = False,
                            passphrase = None, output = None):
        return self._decrypt (["--decrypt", "-u", encoder_key], file, always_trust,
            passphrase, output)
